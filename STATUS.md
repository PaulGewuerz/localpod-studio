...
- [ ] **URGENT: Megaphone episode-create is blocked account-wide** (found 2026-07-27) — every `POST …/episodes` with `backgroundAudioFileUrl` returns 400 "A previously uploaded file is still processing. Please try again shortly.", on every podcast (incl. LocalPod Test) and with known-good audio files. Last successful create: 2026-07-13 15:22 UTC — the block started right around Megaphone's July 14 legacy-API sunset. Nothing visible is stuck: all 300 episodes across all 11 podcasts (paginated, incl. drafts) show `audioFileStatus=success`. Symptom for customers: Approve & Publish / Schedule fail; episodes sit in `approved` (e.g. PBOG "Logisticus Maximus", resolved manually 2026-07-27). **Working workaround (verified live):** create the episode metadata-only (no audio, `draft:true`) → `PUT backgroundAudioFileUrl` (ingest works fine) → poll to `success` → `PUT draft:false` + pubdate. Next steps: (1) open a Megaphone support ticket (mention the July 14 sunset timing); (2) if not resolved quickly, patch `MegaphoneAdapter.publishEpisode` to use the two-step create-then-attach flow (also review schedule/approve delete+recreate paths that assume create-with-audio).
