...
- [x] **RESOLVED (2026-07-29, `abae4c1`): Megaphone episode-create block worked around in code.** Megaphone had begun (intermittently, then persistently) rejecting `POST …/episodes` that include `backgroundAudioFileUrl` with 400 "A previously uploaded file is still processing" — blocking Approve & Publish / Schedule account-wide (started ~Megaphone's July 14 legacy-API sunset; manually rescued PBOG "Logisticus Maximus" 2026-07-27). Fix: `MegaphoneAdapter.publishEpisode` now creates the episode metadata-only (`draft:true`, no audio) then attaches audio + flips `draft:false` in a follow-up `PUT`. Verified the attach path dodges the block and Megaphone still ingests async and publishes when ready — **no polling needed**, so latency/behavior match the old create-with-audio call; all callers (approve, schedule, publish, ad re-sync) fixed transparently. Orphaned empty draft is deleted if the attach fails. Still worth doing: a Megaphone support ticket to understand the root block (the create-with-audio path may recover on its own; the two-step is harmless either way).

- [x] **RESOLVED (2026-09-14): OP3 analytics live end-to-end.** Both go-live steps done: OP3 prefixes added in Megaphone, and a real `OP3_API_TOKEN` is set on Railway (no longer the `preview07ce` sample token). Verified live download data flowing for 4 feeds (PBOG 10 all-time, Texas County Progress, LocalPod Test, Nordonia Hills). **Bug fixed same day (`analytics.js`):** the studio Analytics page showed nothing because the per-episode join used `ep.guid`, but Megaphone leaves episode `guid` null and publishes its `id` as the RSS `<item>` guid — so OP3's `itemGuid` matches `ep.id`. Join now prefers `ep.guid`, falls back to `ep.id`; `monthlyDownloads` now surfaced on the page as a "This Month" stat card. Diagnostic: `backend/scripts/check-op3.js` (re-run anytime to spot-check which feeds OP3 sees). **Still to verify:** prefixes on T8N AM, The Publisher's Edge, and Permian Basin Oil & Gas — OP3 hasn't seen those feeds yet (prefix missing, no traffic since install, or an old/duplicate feed — note PBOG Audio Edition works while Permian Basin Oil & Gas doesn't). Follow-up: add the prefix step to the new-show provisioning checklist.
